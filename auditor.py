...
def calcular_score_qualidade(resultado: pd.DataFrame) -> Tuple[int, str]:
    """Calcula a nota de qualidade geral da base.

    Esta versão separa qualidade dos dados de aderência ao modelo recomendado.
    Assim, relatórios de vendas, marketing, suporte, estoque ou RH não são
    penalizados de forma excessiva apenas por não terem colunas típicas de
    ...
    pesos_por_validacao = {
        "Possível dado sensível": 18,
        "Possível dado pessoal": 12,
        "Duplicidades": 10,
        "Campos vazios": 8,
        "Valores negativos": 8,
        "Valores zerados": 5,
        "Possíveis outliers": 5,
        "Padronização textual": 4,
        "Coluna recomendada": 2,
    }

    pesos_por_criticidade = {
        "Alta": 10,
        "Média": 6,
        "Baixa": 3,
    }
    ...
    for _, linha in alertas.iterrows():
        validacao = str(linha.get("validacao", ""))
        criticidade = str(linha.get("criticidade", "Baixa"))

        peso = pesos_por_validacao.get(
            validacao,
            pesos_por_criticidade.get(criticidade, 3)
        )
...
def calcular_score_aderencia_modelo(resultado: pd.DataFrame) -> Tuple[int, str]:
    """Calcula a aderência às colunas recomendadas.

    Esse score não mede a qualidade geral da base. Ele mostra apenas o quanto
    o arquivo se aproxima do modelo operacional sugerido no projeto.
    """
    colunas = resultado[resultado["validacao"] == "Coluna recomendada"]

    if colunas.empty:
        return 100, "Aderência não avaliada"

    total = len(colunas)
    ausentes = len(colunas[colunas["status"] == "Ausente"])

    score = int(round(((total - ausentes) / total) * 100, 0))

    if score >= 80:
        status = "Alta aderência ao modelo recomendado"
    elif score >= 50:
        status = "Aderência parcial ao modelo recomendado"
    else:
        status = "Baixa aderência ao modelo recomendado"
...
def gerar_diagnostico_executivo(df: pd.DataFrame, auditoria: pd.DataFrame) -> str:
    ...
    texto = [
        ...
        f"A aderência ao modelo recomendado foi {score_aderencia}/100: {status_aderencia}.",
    ]
